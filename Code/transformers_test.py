...
from keras.layers import Flatten, LSTM, Dropout, Dense, Conv2D, Conv1D
...
def LSTM_model(input_shape):
    model = Sequential()
    model.add(LSTM(units=500,return_sequences=True,input_shape=input_shape))
...
def conv_model(input_shape):
    model = Sequential()
    model.add(Conv1D(50,8,1, input_shape = input_shape, activation='relu'))
    model.add(Dropout(0.2))
    model.add(Conv1D(25,8,1, input_shape = input_shape, activation='relu'))
    model.add(Dropout(0.2))
    model.add(Conv1D(50,8,1, input_shape = input_shape, activation='relu'))
    model.add(Dropout(0.2))
...
def coat_model(input_shape):
    return

model_params = []

HEAD_SIZE=46 # Embedding size for attention
NUM_HEADS=85 # Number of attention heads (60)
FF_DIM=80 # Hidden layer size in feed forward network inside transformer, (55)
...
model = build_model(
    input_shape,
    head_size=HEAD_SIZE, # Embedding size for attention
    num_heads=NUM_HEADS, # Number of attention heads (60)
    ff_dim=FF_DIM, # Hidden layer size in feed forward network inside transformer, (55)
    num_transformer_blocks=NUM_TRANSFORMER_BLOCKS,
    mlp_units=[MLP_UNITS], #256
...
model_params.append("input_shape={0}\nhead_size={1}\nnum_heads={2}\nff_dim={3}\nnum_transformer_blocks={4}\nmlp_units={5}\nmlp_dropout={6}\ndropout={7}".format(input_shape, HEAD_SIZE,NUM_HEADS,FF_DIM,NUM_TRANSFORMER_BLOCKS,MLP_UNITS,MLP_DROPOUT,DROPOUT))
...
model.save("C:\\Users\\ngow210\\Downloads\\Time_series_transformers\\transformer_model.h5")
#model_params.to_csv("C:\\Users\\ngow210\\Downloads\\Time_series_transformers\\transformer_model_params.csv", index = False)

np.savetxt("C:\\Users\\ngow210\\Downloads\\Time_series_transformers\\transformer_model_params.txt", np.asarray(model_params), fmt='%s', delimiter = '\n')
